export const startGame = (UI, coachingOverlay, vps, Ammo, wayspotId) => {
  //  Assets
  const assets = [
    {file: 'type1.glb', scale: 1},
    {file: 'type2.glb', scale: 1},
    {file: 'type3.glb', scale: 1},
    {file: 'type4.glb', scale: 1},
    {file: 'type5.glb', scale: 1},
  ]
  let activeShoes = 0
  ...
  */
  // Ring settings
  /*
  const torusVerticalPosition = 1.5
  ...
  */
  ...
  const init = () => {
    ...
    $('.shoes').on('click', function(){
      ...
      activeShoes = $('.shoes').index($(this))
      console.log(activeShoes)
    })

    $('#place-shoe').on('touchstart', function(){
      putShoe()
    })
  ...
  const putShoe = () => {
    ...
    if(intersects.length > 0){
      const shoes = assets[activeShoes].model.clone()
      mainGroup.add(shoes)
      shoes.position.copy(intersects[0].point)
      const randRot = Math.random() * 360
      shoes.rotation.set(0, randRot, 0)
    }
    console.log('put shoe '+(activeShoes+1))
  }
  ...
  const start = () => {
    ...
    // UI
    setTimeout(function () {
      $('.tracking #ball-through-torus-game-ui').fadeIn(500)
...
*/
  const loadAssets = () => {
    for (let i = 0; i < assets.length; i++) {
      const modelFile = '../assets/models/shoes/' + assets[i].file
